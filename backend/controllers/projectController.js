...
const addproject = async (req, res) => {
    ...
    try {
        ...
        if (req.body.userId) {
            ...
            console.log('projects:::' , allproject);
            let findUser = await Project.findOne({
            ...
            if (allproject.length > 0) {
                ...
                if (nameUserFrom?.packageId) {
                    ...
                    if (days <= mypackage?.duration) {
                        ...
                        if (parseInt(nameUserFrom?.addProject) <= 0) {
                            ...
                            const addProject = nameUserFrom?.addProject - 1


                            let user = await User.update({ ...req.body, addProject }, {
                    ...
                    return res.status(200).json({
                        status: 'fail',
                        message: 'You can not add project first buy package'
                    })
                }
...
// 3.get product by id
const getprojectByCity = async (req, res) => {
...
// 3.get product by id
const filterprojectById = async (req, res) => {
    try {
        const { city, title, type, minPrice, maxPrice, category, developers, minArea, maxArea, beds } = req.body;
...
const addprojectimg = async (req, res) => {
...
const addlike = async (req, res) => {
    ...
    try {
        ...
        const getUserFollow = await Project.findOne({
            where: { id: info.id }
        })
        ...
        if (getUserFollow.myLike.includes(info?.userId)) {
            ...
            const newUserFollow = await Project.update({ myLike: myFollow }, {
                where: { id: info.id }
            })
            ...
            res.status(200).json({
                status: 'ok',
                data: newUserFollow,
                message: 'You unlike the project.'
            })
        }
        else {


            const newUserFollow = await Project.update({ myLike: [...getUserFollow?.myLike, info?.userId] }, {
                where: { id: info.id }
            })
